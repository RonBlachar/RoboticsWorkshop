...
def find_colored_points(image, lower_color, upper_color) -> List[Tuple[int, int]]:
    """
...
def process_image(image_path, lower_color, upper_color, output_image_path):
    # Load the image
    image = cv2.imread(image_path)
    
    # Find the colored points in the image
    coordinates = find_colored_points(image, lower_color, upper_color)
...
def process_image(image_path):
    ...
    # Define the color ranges in HSV for pink and blue cubes
    ...
    matrix[pink_mask > 0] = 1
    matrix[blue_mask > 0] = 2
